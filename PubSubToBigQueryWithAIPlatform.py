...
PROJECT_ID = os.getenv('PROJECT_ID','cloudarchitectexam')
...
def prediction_helper(messages):
    ...
    instances = list([json.loads(message.decode('utf-8')) for message in messages])

    # Estimate the sentiment of the 'text' of each tweet
    scores = get_sentiment(
        [instance.get('text') for instance in instances if
         instance.get('text')])
